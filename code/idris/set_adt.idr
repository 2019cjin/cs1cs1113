...
combine (h1 :: t1) l2 = ite (member h1 l2)
                            (combine t1 l2)
                            (combine t1 (h1 :: l2))
-- general for all kinds of lists. when use this for set_union, assume that
-- the lists in mkSet don't have duplicates

-- (my notes) if element in l1 is already in l2, return l2. else return element
-- appended to l2. then go on to next element in l1
...
-- first, need to understand how it works.
-- them implement.
...
-- specification: given a list of a and a list of b, return a list of all a-b pairs.
...
{-
add_element: a -> list (set a) -> set (set a)
add_element a nil = (mkSet (a :: nil)) :: (mkSet nil) :: nil
add_element a (h :: t) = (set_insert a h) :: (add_element a t)

set_powerset (mkSet nil) = (mkSet nil)
set_powerset (mkSet (h :: t)) = set_union (mkSet (add_element h (set_powerset (mkSet t)))) (set_powerset (mkSet t))
-}
-------------------------------------
...
set_eql (mkSet l1) (mkSet l2) = and (subset_elements l1 l2) (subset_elements l2 l1)
...
set_toString (mkSet l) = "{" ++ (stringElements l) ++ "}"
